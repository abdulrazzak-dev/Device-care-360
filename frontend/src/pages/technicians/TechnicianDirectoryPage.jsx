  ...
  return (
    <DashboardLayout>
      <div className="d-flex flex-column flex-sm-row align-items-start align-items-sm-center justify-content-between gap-3 mb-4">
        <div>
          <h3 className="fw-bold mb-1 text-break">Certified Technician Directory</h3>
        ...
        <div className="w-100 w-sm-auto" style={{ minWidth: '220px' }}>
          <select className="form-select form-select-sm w-100" value={specialization} onChange={(e) => setSpecialization(e.target.value)}>
      ...
      {loading ? (
        ...
        <div className="row g-3 g-md-4">
          {technicians.map((tech) => (
            <div className="col-12 col-md-6 col-lg-4" key={tech.id}>
              <div className="card p-3 p-sm-4 border-0 shadow-sm card-hover h-100 d-flex flex-column justify-content-between">
                <div>
                  <div className="d-flex align-items-center justify-content-between mb-2 gap-2 flex-wrap">
                    <span className="badge bg-success bg-opacity-10 text-success fw-bold px-2.5 py-1 rounded-pill">
                      <ShieldCheck size={14} className="me-1 flex-shrink-0" /> {tech.verificationStatus}
                    </span>
                    <div className="d-flex align-items-center text-warning fw-bold small">
                      <Star size={16} className="fill-current me-1 flex-shrink-0" /> {tech.averageRating || 5.0} ({tech.totalReviews || 0})
                    </div>
                  </div>

                  <h5 className="fw-bold mb-1 text-break">{tech.fullName}</h5>
                  <p className="text-muted small mb-3 text-break"><MapPin size={14} className="me-1 flex-shrink-0" /> {tech.serviceAreas?.join(', ') || 'Nationwide'}</p>

                  <div className="mb-3">
                    <span className="text-muted extra-small fw-semibold d-block mb-1">SPECIALIZATIONS:</span>
                    <div className="d-flex flex-wrap gap-1">
                      {tech.specializations?.map((s, idx) => (
                        <span key={idx} className="badge bg-light text-dark border text-break">{s}</span>
                      ))}
                    </div>
                  </div>
                </div>
                ...
                <div className="pt-3 border-top d-flex gap-2">
                  <Link to={`/bookings/new?techId=${tech.id}`} className="btn btn-primary btn-sm w-100 d-flex align-items-center justify-content-center gap-1 fw-bold text-center">
                    <Calendar size={14} className="flex-shrink-0" /> Book Appointment
                  </Link>
                </div>
              </div>
            </div>
